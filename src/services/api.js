...
export async function fetchPost(action) {
  ...
  if(action.mode == 'search') return request(`http://115.159.120.134/api/${payload.source_from[payload.is_search_select_source]}/search?query=${payload.search_key}&page=${payload.is_select_page}`);
